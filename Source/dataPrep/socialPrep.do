...
********************************************************************************
*** (2c) Unpack crime data
********************************************************************************
foreach y in 03 04 05 06 07 08 09 10 11 12 {
  cd "$DAT/Judiciales/20`y'/TablasMicrodatos_20`y'"

	foreach f in pdel20`y' sdel20`y' {
		!dbfdump `f'.DBF --info | grep '^[0-9]*[\.]' | grep [A-Z_] | awk {'printf "%s;", $2'} > `f'.csv
		!echo "" >> `f'.csv
		!dbfdump -fs=';' `f' >> `f'.csv 
		insheet using `f'.csv, names delimit(";") clear
  ...
  cd "$DAT/Judiciales/20`y'/CatalogosMicrodatos_20`y'"

	foreach f in cdel20`y' CENTIDAD {
		!dbfdump `f'.DBF --info | grep '^[0-9]*[\.]' | grep [A-Z_] | awk {'printf "%s;", $2'} > `f'.csv
		!echo "" >> `f'.csv
		!dbfdump -fs=';' `f' >> `f'.csv 
		insheet using `f'.csv, names delimit(";") clear
